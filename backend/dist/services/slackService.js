...
class SlackService {
    async sendInterestedNotification(email) {
        try {
            ...
            logger_1.logger.info(`Sent Slack notification for interested email: ${email.id}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to send Slack notification for email ${email.id}:`, error);
            throw error;
        }
    }
    buildSlackMessage(email) {
    ...
    async testConnection() {
        try {
            ...
            logger_1.logger.info('Slack connection test successful:', auth);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Slack connection test failed:', error);
            return false;
        }
    }
    async sendCustomMessage(channel, message, blocks) {
        try {
            await slackClient.chat.postMessage({
                channel,
                text: message,
            ...
            logger_1.logger.info(`Sent custom Slack message to ${channel}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to send custom Slack message:`, error);
            throw error;
        }
    }
    async getCategoryStatsMessage(stats) {
    ...
    async sendDailyDigest(stats, newInterestedCount) {
        try {
            ...
            await this.sendCustomMessage(process.env.SLACK_CHANNEL_ID || '#general', message);
            logger_1.logger.info('Sent daily digest to Slack');
        }
        catch (error) {
            logger_1.logger.error('Failed to send daily digest:', error);
        }
    }
}
