...
export class AiReplyService {
  ...
  async generateReplySuggestion(emailId: string): Promise<{
    suggestion: string;
    confidence: number;
    reasoning?: string;
  }> {
...
Please generate a professional reply suggestion.`
      ...
      logger.info(`Generated reply suggestion for email ${emailId}`);
  ...
  private findRelevantKnowledge(email: any): string[] {
  ...
  private calculateConfidence(email: any, suggestion: string): number {
    ...
    const matchedTerms = businessTerms.filter(term =>
      emailText.includes(term) && suggestionText.includes(term)
    );
  ...
  async addToKnowledgeBase(fact: string): Promise<void> {
    try {
      this.knowledgeBase.push(fact);
      logger.info('Added new fact to knowledge base:', fact);
    } catch (error) {
  ...
  async generateMultipleSuggestions(emailId: string, count: number = 3): Promise<{
    suggestions: Array<{
      text: string;
      confidence: number;
      tone: string;
    ...
    try {
      ...
      logger.error(`Failed to generate multiple suggestions for email ${emailId}:`, error);
      throw error;
    }
  }

  async refineReply(emailId: string, originalSuggestion: string, feedback: string): Promise<{
    ...
    try {
      ...
      logger.error(`Failed to refine reply for email ${emailId}:`, error);
      throw error;
    }
  }
}
